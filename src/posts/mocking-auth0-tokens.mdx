...
Deploying a change that accidentally borks the permissions on your API — by,
say, publicly exposing data that should only be viewable to a subset of users —
is probably gonna ruin your week. For just that reason, we should all be
terrified of working on code bases that don’t have a robust test coverage of
authentication and permissions. And yet figuring out how to properly mock
different authentication paths in your tests is hardly trivial, requiring a fair
amount of knowledge about how access tokens and authentication providers work.
As a result, many test suites rely on a complicated set of user accounts created
in Auth0 just for the purpose of testing, which is a confusing, flaky, and
brittle setup. Worse yet are the many test suites that just bypass testing
...
tokens so that you can understand how to mock them in your tests. The examples
I’ll provide come from a Django app, but you should be able to reuse the same
concepts in any test suite.
...
(JWKs). Below is an example of what that looks like for a random app but your
app should have an equivalent endpoint at
...
So how can we cut Auth0 out of the permissions process during our tests? Really,
the only places where we need to intervene are the exact same ones where Auth0
intervenes: by issuing the tokens (step 1) and exposing the public key needed to
verify them (step 3). If we do those two things, the app will trust us as the
issuer of the tokens and will handle the rest.
...
Still itching to learn more? Just as confused/stuck as you were? Here are some
resources I found helpful in writing this post:
